    ...
    \section{Results}
        ...
        \subsection[Question 3]{Describe up to five notable anomalies or unusual events you see in the data. Prioritize those issue that are most likely to represent a danger or a serious issue for building operations.}
            ...
            \subsubsection{Danger for Building Operation}
                ...
                With the data in figure \ref{fig:heatmap2}, we can select the column which has the highest sum of R-values. In this data, combination of the column (F\_2\_Z\_16 SUPPLY INLET Temperature) and the data set (f2z2-MC2.csv) has the highest value. Figure \ref{fig:scatter} is written with this combination. As we mentioned herein-above, many points are along the X-axis or the Y-axis. 
                
                $\therefore$ Hence, if the data (F\_2\_Z\_16 SUPPLY INLET Temperature) is decreasing, then the Hazium concentration of Zone 2 on Floor 2 will be increasing; and \textit{vice versa}.
